...
def test_mock_univ2_swapExactTokensForTokens(accounts, mockuniv2, dai, usdt, wbtc, weth, ubd):
    ...
    mockuniv2.setSlippgae(10, {'from':accounts[0]})
    ...
    assert wbtc.balanceOf(accounts[1]) == acc1_wbtc_befroe + AMOUNT_OUT_WBTC - int(AMOUNT_OUT_WBTC * mockuniv2.slippagePercent() / 100)

def test_mock_univ2_swapExactTokensForTokens_back(accounts, mockuniv2, dai, usdt, wbtc, weth, ubd):
    
    wbtc.mint(accounts[1], AMOUNT_IN_USDT, {'from':accounts[1]})
    ...
    assert usdt.balanceOf(accounts[1]) == acc1_usdt_before + AMOUNT_IN_USDT - int(AMOUNT_IN_USDT * mockuniv2.slippagePercent() / 100)
...
def test_mock_univ2_swapExactTokensForETH(accounts, mockuniv2, dai, usdt, wbtc, weth, ubd):
    ...
    assert usdt.balanceOf(accounts[0]) == acc0_usdt_before - AMOUNT_IN_USDT
    assert accounts[1].balance() == acc1_eth_befroe + AMOUNT_OUT_ETH - int(AMOUNT_OUT_ETH * mockuniv2.slippagePercent() / 100)   
...
def test_mock_univ2_swapExactETHForTokens(accounts, mockuniv2, dai, usdt, wbtc, weth, ubd):
    ...
    assert dai.balanceOf(accounts[1]) == acc1_dai_before + AMOUNT_IN_DAI/10 - int( (AMOUNT_IN_DAI/10) * mockuniv2.slippagePercent() / 100)   
